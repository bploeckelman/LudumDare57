 ...
 *         can be accessed through {@link Type#getVariant(String)}, if such a combo exists
 *     </li>
 *     <li>
...
public class Fonts extends AssetContainer<Fonts.Type, Fonts.FontVariantMap> {
    ...
    public static AssetContainer<Type, Fonts.FontVariantMap> container;
    ...
    public enum Type implements AssetEnum<FontVariantMap> {
        ...
        public BitmapFont getVariant(String variantName) {
            var variant = fontByVariantName.get(variantName);
            return Optional.ofNullable(variant).orElseGet(() -> {
                Util.log(TAG, Stringf.format("failed to find font %s variant '%s', using 'default' instead", name(), variantName));
                return getDefault();
            });
        }
     ...
     * be looked up by {@link Type#getVariant}
     */
    @Override
