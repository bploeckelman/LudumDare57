...
public abstract class AssetContainer<T extends Enum<T> & AssetEnum<ResourceType>, ResourceType> {
    ...
    public ResourceType get(T assetType) {
        if (assetType == null) {
            Util.log(containerClassName, Stringf.format("Unable to get %s, null asset type provided", resourceTypeClass.getSimpleName()));
            return null;
        }

        var resource = resources.get(assetType);
        if (resource == null) {
            Util.log(containerClassName, Stringf.format("%s resource not found for asset type '%s'", resourceTypeClass.getSimpleName(), assetType.name()));
        }
        return resource;
    }
}
